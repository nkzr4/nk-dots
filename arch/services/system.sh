...
set_sys_pacman() {
    ...
    check_sys_vendor
    pacman -Sy --noconfirm base-devel grub-btrfs os-prober mtools networkmanager network-manager-applet dialog wpa_supplicant dosfstools openssh git pipewire pipewire-pulse pipewire-jack wireplumber pavucontrol bluez bluez-utils xdg-utils xdg-user-dirs alsa-utils inetutils $CPU man-db man-pages texinfo ipset firewalld acpid hyprland kitty uwsm thunar waybar wofi xdg-desktop-portal-hyprland qt5-wayland qt6-wayland polkit-kde-agent grim slurp inotify-tools gnome-keyring noto-fonts ttf-font-awesome ttf-jetbrains-mono-nerd
}
